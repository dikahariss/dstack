...
A skill catalog can target many AI agents at once: Claude Code, Codex,
Kiro, OpenCode, and so on. A "render for everyone" default is
tempting because the port abstraction makes it look free.

It is not free. Costs that scale linearly with host count:

1. **Disk usage.** Each host adapter writes its own copy of every
   skill. Ten hosts means roughly ten times the on-disk footprint.
2. **Build time.** The generator loops over every skill, once per
   host. Each loop reads templates, formats output, and writes files.
3. **Mental load.** A new user sees N directories that look like
   waste. Each one prompts "do I need this?"

The current user runs only Claude Code. Until a second host has a
named user behind it, building adapters for every plausible host is
speculation.
...
- dstack gives up the "works for everyone" install pitch. It is more
