...
A typical template-driven render pipeline runs on the order of 14
steps per template:
...
Each step transforms the data in some way. The output is hard to
predict from the input because resolvers reference each other and
share state.
...
Steps the contrast pipeline performs that dstack deliberately omits:
...
Reason: resolver systems start small and grow. Four functions become
seventeen over time. A plugin system attracts plugins.
...
- A mature resolver-driven pipeline can easily reach 17 files and
  about 4 500 lines. Each resolver has a reason for existing; the
  total cost — read-time complexity, test surface, debugging path —
  is what makes the flat approach attractive at this scale.
