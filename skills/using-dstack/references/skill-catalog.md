...
| `auditing-video` | A video FILE of ANY length needs a structured audit plus a dataset the user keeps — per-video fact row, per-second/per-shot/OCR tables that concatenate across videos; or several audits merged into a corpus; or video measurements prepared for a warehouse / ML feature store. Answers "is this any good". Ten of its 36 items are feed mechanics and gate off by `format_class` outside short vertical. "analyze this video", "audit video", "why isn't this video performing", "hook analysis", "video dataset". Local files only — NOT a platform URL, NOT judging a running app, and NOT rebuilding a video as prompts. Renamed from `auditing-short-video`, which stays a trigger. | → `verifying-before-done` before claiming the audit is complete; → `multi-persona-review` if the video needs several expert points of view. |
| `reverse-engineering-video` | A video FILE must be taken apart and rebuilt as generation prompts — shot bible, per-shot deep rows, an edit list, an on-screen-text table, and prompts for video, image, speech, sound effects and music. Answers "how was this made". Detects shots first, then samples 3–5 fps INSIDE shots (never globally — a 20-minute file at a flat 4 fps exceeds any context, and at 1 fps a dolly-in and a cut are identical). "reverse engineer this video", "recreate this video", "video to prompt", "scene breakdown", "how was this video made". Local files only. Not `/auditing-video` — that judges, this rebuilds; the same file can go through both. | → `dispatching-parallel-agents` once the frame budget exceeds one context; → `generating-images` to render the stills the prompts describe; → `verifying-before-done` before claiming the package is complete. |
| `researching-facts` | An answer depends on the world's current state — a price, quota, version, date, regulation, or who shipped what — or a claim must ship with a citation and a retrieval date. Runs the host's own `WebSearch` and the Brave Search API **in the same message** so two independent indexes are queried in parallel, fuses the variants by reciprocal rank, then fetches the primary source; an aggregator quoting a doc counts as zero sources. Discloses when the fan-out degraded to one engine. Brave is metered ($5/1k requests), so the fan-out is budgeted. "research this", "riset", "cari data/fakta", "is this still true", "cite sources", "latest version of", "brave search". Not for an academic corpus (`literature-search`) and not for a library's own docs (Context7). | → `verifying-before-done` before the claim ships; feeds `discovering-requirements` (constraint evidence) and `writing-specs` (a cited external contract). |
| `generating-images` | An image must be CREATED because nothing can be found and nothing can be drawn deterministically — a scene no stock library holds, a cover, a placeholder photo, stills for a video. Drives an agent CLI already paid for (`codex exec` → built-in `image_gen`; `agy --print` → `generate_image`) through a bundled script that enforces the JSON-schema contract, copies the asset out of the CLI's own cache, and reads the real dimensions from the file's bytes. Engine is chosen on INTENT, never as a fallback chain: codex for highest fidelity and lossless PNG, agy for locally/culturally specific subjects and for continuity across shots via a reference image. Neither engine honours a requested size — measured, both round to their own budget — so the reply must carry the real measured size AND the ceiling (codex ≈76%, agy ≈51% of a 1080×1920 frame). Metered 4K API routes exist and stay behind an explicit paid decision. "generate an image", "buatkan gambar", "bikin ilustrasi", "text to image", "nano banana", "gpt-image". Not for charts, diagrams, or screen layouts. | → `verifying-before-done` before claiming the asset is delivered; charts go to `/dataviz`, diagrams to `/diagramming-architecture`, screens to `/wireframing-interfaces`. |
...
- **Answering from the web:** `researching-facts` (parallel engines → merge →
  fetch the source) → `verifying-before-done`. Academic corpora take the
  literature chain instead, not as well.
