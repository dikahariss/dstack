...
| `auditing-short-video` | A short-form video FILE (Reel/TikTok/Shorts, mp4/webm/mov) needs a structured audit plus a dataset the user keeps — per-video fact row, per-second/per-shot/OCR tables that concatenate across videos; or several audits merged into a corpus; or video measurements prepared for a warehouse / ML feature store. "analisis video ini", "audit video", "kenapa video ini tidak perform", "hook analysis", "video dataset". Local files only — NOT a platform URL, and NOT judging a running app. | → `verifying-before-done` before claiming the audit is complete; → `multi-persona-review` if the video needs several expert points of view. |
