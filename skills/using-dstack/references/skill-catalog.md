...
| `multi-persona-review` | ONE artifact reviewed from several expert points of view to surface more DISTINCT issues, then closed on a decision someone owns. Mandatory Dreamer / Realist / Critic trio (Disney Creativity Strategy) dispatched blind and in parallel, at most two specialists on top, run over **three capped iterations** wearing Six Thinking Hats — diverge, converge, conditional Go/No-Go. Findings reconciled by union, decisive claims verified against sources, ending in verdicts, a work-assignment table, and disagree-and-commit with a dissent register. "panel review", "cross review", "PoV senior data architect", "devil's advocate", "red team this", "six thinking hats". Raises coverage, NOT factual accuracy — accuracy comes from the verification pass, not from more seats. | Uses `dispatching-parallel-agents` mechanics; ← `brainstorm` when no artifact exists yet; pairs with `running-uat` and `requesting-code-review`; → `writing-plans` carries the assignment table. |
